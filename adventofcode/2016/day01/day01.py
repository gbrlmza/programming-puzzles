...
def main():
    [x,y] = [0,0] # initial x,y coordinates
    direction = 90 # degrees, 90º is north, the starting position
    visited_locations = ['0,0'] # the initial location

    with open('input.txt', 'r') as file:
        instructions = [x.strip() for x in file.read().strip().split(',')]
    
    for move in instructions:
        turn = 90 if move[0] == 'L' else -90
        walk = int(move[1:])
    ...
    print("The Easter Bunny HQ is {} blocks away".format(distance))
