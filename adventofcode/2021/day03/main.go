...
func partOne(input []string) (result int) {
	...
	// get most common values for each position
...
func partTwo(input []string) (result int) {
	...
	oxigen, _ := strconv.ParseInt(oxigenSlice[0], 2, 64)
	co2, _ := strconv.ParseInt(co2Slice[0], 2, 64)
	return int(oxigen * co2)
}
...
func bitCriteria(mostCommon bool, pos int, input []string) []string {
	...
	want := "1"
	if (mostCommon && common < 0) || (!mostCommon && common >= 0) {
		want = "0"
	}

	var result []string
	for _, num := range input {
		if string(num[pos]) == want {
